...
@section('title','Chi Tiết Sản Phẩm Giảm Giá')
@section('main')
<link rel="stylesheet" href="css/details.css">
<div id="wrap-inner">
	<div id="product-info">
		<div class="clearfix"></div>
		<h3>{{$itemdiscount->discount_name}}</h3>
		<div class="row">
			<div id="product-img" class="col-xs-12 col-sm-12 col-md-3 text-center">
				<img height="340px" src="{{asset('library/storage/app/avatar/'.$itemdiscount->discount_img)}}">
			</div>
			<div id="product-details" class="col-xs-12 col-sm-12 col-md-9">
				<p>Giá: <span class="price">{{number_format($itemdiscount->discount_price,0,',','.')}}VNĐ</span></p>
				<p>Tác Giả: {{$itemdiscount->discount_author}}</p> 
				<p>Bảo hành: {{$itemdiscount->discount_warranty}}</p> 
				<p>Phụ kiện: {{$itemdiscount->discount_accessories}}</p>
				<p>Tình trạng: {{$itemdiscount->discount_condition}}</p>
				<p>Khuyến mại: {{$itemdiscount->discount_promotion}}%</p>
				<p>Sản phẩm : @if($itemdiscount->discount_status==1)Còn hàng @else Hết hàng @endif</p>
				<form action="">
						
					<p class="add-cart text-center">
						<a href="{{asset('cart/adddiscount/'.$itemdiscount->discount_id)}}">Đặt hàng online</a>
					</p>
				</form>
				...
				<div class="tab-content">
					<div role="tabpanel" class="tab-pane fade active in " id="content_detail-1" style="font-size: 20px; font-weight: bold">
						<div class="med-cart__desc ">
							<p class="text-justify" >{!!$itemdiscount->discount_description!!}</p>
						</div> 
					</div>
					<div role="tabpanel" class="tab-pane fade active in" id="list_content-1" style="font-size: 20px; color: red; font-weight: bold">
						<div class="med-cart__desc">
							<p class="text-justify">
								<ul>
									<li>{!!$itemdiscount->discount_accessories!!}</li>
								</ul>
							</p>
						</div> 
					</div>
					<div role="tabpanel" class="tab-pane fade active in" id="rate_list">
						<div class="med-cart__desc">
							...
							<div id="comment-list">
								...
								<ul>
									<li class="com-title">
										{{$comment->comdiscount_name}}
										<br>
										<span>{{date('d/m/Y H:i',strtotime($comment->created_at))}}</span>	
									</li>
									<li class="com-details">
										{{$comment->comdiscount_content}}
									</li>
								</ul>
								@endforeach
							</div>
