...
@section('title','Sửa Danh Mục Giới Thiệu')
@section('main')
					...
					<div class="panel panel-primary">
						...
						<div class="panel-body">
							...
							<div class="form-group">
    							...
    							<input required type="text" name="name" class="form-control" placeholder="Tên danh mục..." value="{{$intro->intro_name}}">
							</div>
							<div class="form-group">
								<label>MenuList danh mục:</label>
    							<select required name="menu" class="form-control">
									@foreach ($menus as $menu)
										<option value="{{$menu->menu_id}}" @if($intro->intro_menu == $menu->menu_id) selected @endif>{{$menu->menu_name}}</option>
									@endforeach
							...
							<div class="form-group">
    							<a href="{{asset('admin/introduce')}}" class="form-control btn btn-danger">Hủy bỏ</a>
							</div>
							{{csrf_field()}}
							</form>
						</div>
