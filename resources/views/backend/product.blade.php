								...
								<table class="table table-bordered" style="margin-top:20px;">				
									...
									@foreach ($productlist as $product)
										<tr>
											...
											<td>{{$product->prod_slug}}</td>
											<td>{{$product->prod_author}}</td>
											<td>{{number_format($product->prod_price,0,',','.')}} VND</td>
											<td>
												<img width="200px" src="{{asset('library/storage/app/avatar/'.$product->prod_img)}}" class="thumbnail">
											</td>
											<td>{{$product->cate_name}}</td>
											<td>
												<a href="{{asset('admin/product/edit/'.$product->prod_id)}}" class="btn btn-warning"><i class="fas fa-pencil-alt" aria-hidden="true"></i> Sửa</a>
