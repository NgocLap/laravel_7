					...
					<div class="panel panel-primary">
						...
						<div class="panel-body">
							...
							<div class="form-group">
    							...
    							<input required type="text" name="name" class="form-control" placeholder="Tên danh mục..." value="{{$infor->cate_name}}">
							</div>
							<div class="form-group">
								<label>MenuList danh mục:</label>
    							<select required name="menu" class="form-control">
									@foreach ($menus as $menu)
										<option value="{{$menu->menu_id}}" @if($infor->infor_menu == $menu->menu_id) selected @endif>{{$menu->menu_name}}</option>
									@endforeach
								</select>
							</div>
							<div class="form-group">
							...
							<div class="form-group">
    							<a href="{{asset('admin/information')}}" class="form-control btn btn-danger">Hủy bỏ</a>
							</div>
							{{csrf_field()}}
							</form>
						</div>
